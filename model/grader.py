...
class Grader:
    def __init__(self, evaluator_type: LineSegmentEvaluator, ideal_solution_path: str, predicted_solution_path: str) -> None:
        self.evaluator: LineSegmentEvaluator = evaluator_type
    ...
    def print_distances(self) -> None:
        ...
        for solution in self.solution_line_segments: 
            for file_name, submission_line_seg in solution.items():
                ideal_solution_line_seg: list[LineSegment] = self._find_solution_in_ideal(self.ideal_line_segments, file_name)

                if not ideal_solution_line_seg or not submission_line_seg:
                    continue

                min_distances: list[float] = self._calculate_predicted_from_ideal_distance(file_name, ideal_solution_line_seg, submission_line_seg, t)
                all_min_distances.extend(min_distances)
        t.add_row(["Total", "", sum(all_min_distances)])
        print(t.draw())

    def _calculate_predicted_from_ideal_distance(self, file_name: str,
    ...
    def _find_matching_lines(self, ideal_line: LineSegment, submission_line_seg: list[LineSegment]) -> list[float]:
        if isinstance(ideal_line,LineSegment) and isinstance(submission_line_seg, (list, list[LineSegment])):
            return [self.evaluator.distance_from_ideal(ideal_line, submission_line) for submission_line in submission_line_seg]
        return []
